CREATE OR REPLACE FUNCTION public.get_time_agregado(p_gestor_id uuid)
 RETURNS TABLE(corretor_id uuid, nome text, avatar_url text, segmento_principal text, total_leads integer, total_recebidos integer, sem_tarefa integer, atrasados integer, em_dia integer, para_hoje integer, sem_contato_5d integer, negocios integer, vgv_pipeline numeric, conversao_pct numeric, alerta_principal text)
 LANGUAGE plpgsql
...
DECLARE
  ...
  v_month_start_brt timestamptz := date_trunc('month', now() AT TIME ZONE 'America/Sao_Paulo') AT TIME ZONE 'America/Sao_Paulo';
  v_month_start_date date := date_trunc('month', now() AT TIME ZONE 'America/Sao_Paulo')::date;
...
BEGIN
  ...
  agg_sem_contato AS (
    SELECT
      la.corretor_auth_id,
      count(*) FILTER (
        WHERE COALESCE(la.ultima_acao_at, la.created_at) < (now() - INTERVAL '5 days')
      )::int AS sem_contato_5d
    FROM leads_ativos la
    GROUP BY la.corretor_auth_id
  ),
  agg_negocios AS (
    SELECT
      tp.corretor_auth_id,
      ...
      COALESCE(
        SUM(COALESCE(n.vgv_final, n.vgv_estimado)) FILTER (
          WHERE n.data_assinatura IS NOT NULL
            AND n.data_assinatura >= v_month_start_date
        ),
        0
      )::numeric AS vgv_pipeline
  ...
  conv_num AS (
    SELECT n.auth_user_id AS corretor_auth_id, count(*)::int AS assinados_mes
  ...
  conv_den AS (
    SELECT pl.corretor_id AS corretor_auth_id, count(*)::int AS leads_mes
    FROM public.pipeline_leads pl
    JOIN team t ON t.corretor_auth_id = pl.corretor_id
    WHERE pl.created_at >= v_month_start_brt
    GROUP BY pl.corretor_id
  ),
  agg_recebidos AS (
    SELECT pl.corretor_id AS corretor_auth_id, count(*)::int AS total_recebidos
    FROM public.pipeline_leads pl
    JOIN team t ON t.corretor_auth_id = pl.corretor_id
    WHERE pl.created_at >= v_month_start_brt
    GROUP BY pl.corretor_id
  ...
  SELECT
    ...
    COALESCE(ar.total_recebidos, 0) AS total_recebidos,
    ...
    CASE
      WHEN COALESCE(cd.leads_mes, 0) > 0
        THEN ROUND((COALESCE(cn.assinados_mes, 0)::numeric / cd.leads_mes::numeric) * 100, 1)
      ELSE NULL
  ...
  LEFT JOIN agg_recebidos ar ON ar.corretor_auth_id = tp.corretor_auth_id
