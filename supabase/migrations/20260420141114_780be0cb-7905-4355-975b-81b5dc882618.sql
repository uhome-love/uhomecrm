...
BEGIN
  ...
  -- ATTEMPT 1: Round-robin estrito 1 a 1 (apenas ultima_distribuicao_at)
  ...
  ORDER BY 
    sub.ultima_distribuicao_at ASC NULLS FIRST,  -- Round-robin: quem esperou mais recebe primeiro
    sub.cred_created ASC                          -- Desempate só quando timestamps idênticos
  LIMIT 1;

  -- ATTEMPT 2: Force fallback (CEO dispatch only) - mesma regra round-robin
  IF v_chosen_auth_id IS NULL AND p_force = true THEN
    ...
    IF v_total_fila > 0 AND v_total_blocked >= v_total_fila THEN
      ...
      ORDER BY 
        sub.ultima_distribuicao_at ASC NULLS FIRST,
        sub.cred_created ASC
      LIMIT 1;
  ...
  -- Sync timestamp across ALL rows of same broker on same day (garante round-robin entre segmentos/janelas)
  UPDATE public.roleta_fila
