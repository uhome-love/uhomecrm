...
DECLARE
  ...
  v_avulso_segmento_id UUID := '9948f523-29f4-46a7-bc1b-81ff8bb8dd50'; -- S1 Moradia (Avulso unificado em Moradia)
...
BEGIN
  ...
  -- Roteamento explícito por ORIGEM: Imovelweb / Site → S1 Moradia
  ...
  IF NOT v_matched_campaign AND v_emp_lower <> '' THEN
    ...
    ORDER BY
      (lower(trim(empreendimento)) = v_emp_lower) DESC,
      length(empreendimento) DESC
    LIMIT 1;
