...
-- Fix: Include 'dia_todo' credentialed brokers in ANY window match
...
BEGIN
  ...
  -- Check if today is a holiday (same list as frontend)
  v_is_holiday := v_today_date IN ('2026-04-03'::date);

  -- Determine janela
  IF p_janela IS NOT NULL THEN
    v_target_janela := p_janela;
  ELSIF v_is_sunday OR v_is_holiday THEN
  ...
  WHERE c.data = v_today_date
    ...
    -- Window matching: dia_todo matches ANY window, and any window matches dia_todo target
    AND (
      v_target_janela = 'dia_todo' 
  ...
  ORDER BY
    -- 1st: fewest leads IN THIS WINDOW
    COALESCE((
    ...
    -- 2nd: fewest leads in this segment within window
