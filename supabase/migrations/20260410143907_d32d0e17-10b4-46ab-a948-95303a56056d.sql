...
-- Fix round-robin: sync ultima_distribuicao_at across ALL rows of the same broker

-- 1) Recreate distribuir_lead_atomico with the fix
...
BEGIN
  ...
  -- ATTEMPT 2: Force fallback (CEO dispatch only)
  ...
  -- FIX: Sync timestamp across ALL rows of same broker on same day
  UPDATE public.roleta_fila
...
-- 2) Recreate trg_auto_distribute_lead with the same fix
