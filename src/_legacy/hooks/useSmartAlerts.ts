  ...
  useEffect(() => {
    ...
    const check = async () => {
      ...
      try {
        ...
        let stalledQuery = supabase.from("negocios").select("id, nome_cliente, fase, updated_at").lt("updated_at", sevenDaysAgo).not("fase", "eq", "ganho").neq("status", "perdido");
        ...
        if (stalledNegocios && stalledNegocios.length > 0) {
          result.push({
            ...
            action: { label: "Ver negócios", url: "/pipeline-negocios" },
          });
          badgeCounts["/pipeline-negocios"] = (badgeCounts["/pipeline-negocios"] || 0) + stalledNegocios.length;
        }
      } catch (e) {
