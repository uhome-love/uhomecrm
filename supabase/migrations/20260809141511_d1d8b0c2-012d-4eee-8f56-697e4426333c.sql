CREATE OR REPLACE FUNCTION public.reengajamento_reset_rastreio(p_lead_id uuid, p_empreendimento text, p_template text)
 RETURNS void
 LANGUAGE sql
 SECURITY DEFINER
...
AS $function$
  UPDATE public.pipeline_leads
     SET campanha = p_empreendimento,
         formulario = NULL,
         conjunto_anuncio = NULL,
         anuncio = NULL,
         origem_detalhe = 'Reengajamento ' || p_template,
         updated_at = now()
   WHERE id = p_lead_id
     AND COALESCE(trim(p_empreendimento), '') <> '';
$function$;
