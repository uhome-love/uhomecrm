...
BEGIN
  ...
  v_is_casatua_canoas := v_tpl ILIKE '%casatuacanoas%' OR v_tpl ILIKE '%casa tua canoas%' OR v_tpl ILIKE '%casa_tua_canoas%';
  v_is_casatua := (NOT v_is_casatua_canoas) AND (v_tpl ILIKE '%casatua%' OR v_tpl ILIKE '%casa tua%' OR v_tpl ILIKE '%casa_tua%');
  ...
  v_is_awa := v_tpl ~* '(^|[^a-z])awa([^a-z]|$)';

  IF v_is_casatua_canoas THEN
    v_seg_id := v_seg_moradia; v_empreend := 'Casa Tua Canoas'; v_foco_label := 'Casa Tua Canoas';
  ...
  ELSIF v_is_awa THEN
    v_seg_id := v_seg_invest; v_empreend := 'AWA'; v_foco_label := 'AWA (Investimento)';
  ...
  v_emp_mudou := COALESCE(trim(v_empreend),'') <> ''
                 AND lower(trim(v_empreend)) IS DISTINCT FROM lower(trim(COALESCE(v_lead.empreendimento,'')));
  ...
  v_nova_obs := concat(
    ...
    CASE WHEN v_emp_mudou AND COALESCE(trim(v_lead.empreendimento),'') <> ''
         THEN ' Antes: ' || v_lead.empreendimento || '.' ELSE '' END,
  ...
  UPDATE public.pipeline_leads
     SET reengajamento_status = 'respondeu_sim',
         ...
         campanha = CASE WHEN v_emp_mudou THEN v_empreend ELSE campanha END,
         formulario = CASE WHEN v_emp_mudou THEN NULL ELSE formulario END,
         conjunto_anuncio = CASE WHEN v_emp_mudou THEN NULL ELSE conjunto_anuncio END,
         anuncio = CASE WHEN v_emp_mudou THEN NULL ELSE anuncio END,
         origem_detalhe = CASE WHEN v_emp_mudou THEN 'Reengajamento ' || v_tpl ELSE origem_detalhe END,
  ...
  RETURN jsonb_build_object(
    ...
    'empreendimento', v_empreend,
    'empreendimento_atualizado', v_emp_mudou,
...
BEGIN
  ...
  v_emp_txt := NULL;
  IF v_tpl ILIKE '%casatuacanoas%' OR v_tpl ILIKE '%casa tua canoas%' OR v_tpl ILIKE '%casa_tua_canoas%' THEN
...
DECLARE
  v_stage_novo_lead uuid := 'd3843b2f-2fa1-4c31-9129-4eb0ed21f019';
  v_stage_descarte uuid := '1dd66c25-3848-4053-9f66-82e902989b4d';
  v_seg_moradia uuid := 'c8b24415-3dc1-4f65-aae1-f308ef02cb7a'; -- S1 - Moradia
  v_seg_invest  uuid := 'dd96ad01-7e76-40e9-8324-211166168b26'; -- S2 - Investimento
  v_seg_alto    uuid := '5e930c09-634d-40e1-9ccc-981b0a4eae74'; -- S3 - Alto Padrão
  v_seg_mcmv    uuid := '5311bbbb-0000-4000-8000-000000000003'; -- S4 - MCMV
  v_oa record;
  v_tpl text := COALESCE(NULLIF(trim(p_template_name), ''), 'reengajamento');
  v_is_casatua boolean;
  v_is_vivid boolean;
...
BEGIN
  ...
  IF v_is_casatua THEN
    ...
    v_foco_label := 'Casa Tua';
  ELSIF v_is_vivid THEN
    v_empreend := 'Vivid Terrace';
    v_seg_id := v_seg_moradia;
    v_foco_label := 'Vivid Terrace';
  ELSIF v_is_flow THEN
    v_empreend := 'Flow';
    v_seg_id := v_seg_mcmv;
    v_foco_label := 'Flow (MCMV)';
  ELSIF v_is_lake THEN
