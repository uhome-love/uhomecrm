-- 1) Atualiza RPC para reutilizar lock existente do mesmo corretor
...
RETURNS TABLE(
  ...
  motivo_descarte_raw text, reengajamento_status_raw text, locked_until timestamp with time zone,
  ...
  lead_stage_changed_at timestamp with time zone,
  lead_score integer, lead_temperatura text,
...
BEGIN
  IF p_lock THEN
    -- Reutiliza lock ativo do MESMO corretor (evita acumular reservas)
...
-- 2) Backfill: libera locks órfãos ativos da sessão em curso
UPDATE public.oferta_ativa_fila
