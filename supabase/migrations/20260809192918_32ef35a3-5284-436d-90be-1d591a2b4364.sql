...
DECLARE
  ...
  v_vis_periodo jsonb; v_vis_coorte jsonb;
BEGIN
  ...
  SELECT jsonb_build_object(
    'criada',     count(*) FILTER (WHERE conta_realizada OR conta_no_show OR conta_a_realizar),
    'realizada',  count(*) FILTER (WHERE conta_realizada),
    'no_show',    count(*) FILTER (WHERE conta_no_show),
    'a_realizar', count(*) FILTER (WHERE conta_a_realizar)
  ) INTO v_vis_periodo
  FROM v_fato_visita
  WHERE corretor_auth_id = ANY(v_team_auth)
    AND data_criacao BETWEEN p_start AND p_end;
  ...
  FROM v_fato_visita vf
  WHERE vf.corretor_auth_id = ANY(v_team_auth)
    AND vf.data_criacao BETWEEN p_start AND p_end
    AND vf.pipeline_lead_id IN (
  ...
  RETURN jsonb_build_object(
    'periodo_todo', v_periodo || jsonb_build_object('visitas', COALESCE(v_vis_periodo, '{}'::jsonb)),
    'coorte',       v_coorte   || jsonb_build_object('visitas', COALESCE(v_vis_coorte, '{}'::jsonb)),
