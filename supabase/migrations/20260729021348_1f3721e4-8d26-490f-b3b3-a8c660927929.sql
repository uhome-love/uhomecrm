...
CREATE OR REPLACE FUNCTION public._capi_sha256_norm(p_input text)
RETURNS text
LANGUAGE sql
IMMUTABLE
...
AS $$
  SELECT CASE
    ...
    ELSE encode(extensions.digest(lower(btrim(public.unaccent(p_input))), 'sha256'), 'hex')
  END
$$;

GRANT EXECUTE ON FUNCTION public._capi_sha256_norm(text) TO service_role, authenticated;
...
BEGIN
  ...
  -- Fallback fixo Porto Alegre / RS. Overrides futuros por empreendimento entram aqui.
