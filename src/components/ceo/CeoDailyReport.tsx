...
import { Download, FileText, Loader2, Printer, ChevronDown } from "lucide-react";
  ...
  return (
    <div className="space-y-4">
      ...
      <div ref={reportRef} className="space-y-4 bg-background">
        ...
        {/* Collapsible detail: Consolidado + Equipes */}
        <Collapsible open={detailOpen} onOpenChange={setDetailOpen}>
          <CollapsibleTrigger asChild>
            <button className="w-full flex items-center justify-between px-4 py-2.5 rounded-lg border border-border bg-muted/30 hover:bg-muted/50 transition-colors">
              <span className="text-xs font-semibold text-foreground">🏢 Consolidado Geral + Equipes</span>
              <ChevronDown className={`h-4 w-4 text-muted-foreground transition-transform ${detailOpen ? "rotate-180" : ""}`} />
            </button>
          </CollapsibleTrigger>
