...
const S1 = { icon: "🏠", color: "#60A5FA", bg: "rgba(96,165,250,0.08)", border: "rgba(96,165,250,0.25)", order: 1 }; // Moradia (azul)
const S2 = { icon: "📈", color: "#4ADE80", bg: "rgba(74,222,128,0.08)", border: "rgba(74,222,128,0.25)", order: 2 }; // Investimento (verde-claro)
const S3 = { icon: "🏆", color: "#F0B95A", bg: "rgba(240,185,90,0.08)", border: "rgba(240,185,90,0.25)", order: 3 }; // Alto Padrão (dourado)
const S4 = { icon: "🏘️", color: "#16A34A", bg: "rgba(22,163,74,0.08)", border: "rgba(22,163,74,0.25)", order: 4 }; // MCMV (verde)

const SEGMENTO_VISUALS: Record<string, { icon: string; color: string; bg: string; border: string; order: number }> = {
  // Modelo canônico (4 segmentos) — jul/2026
  "s1 - moradia": S1,
  "s2 - investimento": S2,
  "s3 - alto padrão": S3,
  "s3 - alto padrao": S3,
