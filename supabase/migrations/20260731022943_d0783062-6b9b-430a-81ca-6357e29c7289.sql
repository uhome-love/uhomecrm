CREATE OR REPLACE VIEW public.v_kpi_negocios AS
 SELECT COALESCE(n.auth_user_id, p.user_id) AS auth_user_id,
    ...
    n.corretor_id AS profile_id,
        CASE
            WHEN ((n.fase = ANY (ARRAY['em_negociacao'::text, 'contrato'::text])) AND (n.status = 'ativo'::text)) THEN 1
            ELSE 0
        END AS conta_proposta,
        CASE
            WHEN ((n.fase = 'ganho'::text) AND (n.status = 'ativo'::text)) THEN 1
            ELSE 0
        END AS conta_venda,
        CASE
            WHEN (n.status = 'perdido'::text) THEN 1
            ELSE 0
        END AS conta_perdido,
    1.0 AS fator_split,
...
UNION ALL
 SELECT pp.corretor_principal_id AS auth_user_id,
    ...
    round(((n.vgv_estimado * (pp.divisao_principal)::numeric) / 100.0), 2) AS vgv_estimado,
        CASE
            WHEN (n.vgv_final IS NOT NULL) THEN round(((n.vgv_final * (pp.divisao_principal)::numeric) / 100.0), 2)
            ELSE NULL::numeric
        END AS vgv_final,
...
UNION ALL
 SELECT pp.corretor_parceiro_id AS auth_user_id,
    ...
    round(((n.vgv_estimado * (pp.divisao_parceiro)::numeric) / 100.0), 2) AS vgv_estimado,
        CASE
            WHEN (n.vgv_final IS NOT NULL) THEN round(((n.vgv_final * (pp.divisao_parceiro)::numeric) / 100.0), 2)
            ELSE NULL::numeric
        END AS vgv_final,
...
AS $function$
  ...
  neg AS (
    SELECT n.auth_user_id,
      ...
      -- vendas = vendas ASSINADAS no período (mesma janela do vgv_assinado), negócio distinto
      COUNT(DISTINCT CASE WHEN n.data_assinatura BETWEEN p_start AND p_end AND n.conta_venda = 1 THEN n.id END) AS vendas,
