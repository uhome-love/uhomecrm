...
-- 1) Coluna motivo_pendencia
ALTER TABLE public.pipeline_leads
  ADD COLUMN IF NOT EXISTS motivo_pendencia TEXT;

COMMENT ON COLUMN public.pipeline_leads.motivo_pendencia IS
  'Motivo pelo qual o lead está em pendente_distribuicao. Valores esperados: empreendimento_inativo, no_broker_available, no_fila_active, timeout_multiplo.';

-- 2) Atualiza distribuir_lead_atomico para bloquear empreendimento inativo
...
BEGIN
  ...
  IF v_emp_canonico_id IS NOT NULL AND NOT p_force THEN
    SELECT ativo INTO v_emp_ativo
    FROM public.empreendimentos_canonicos
    WHERE id = v_emp_canonico_id;
    ...
    IF COALESCE(v_emp_ativo, TRUE) = FALSE THEN
      ...
      INSERT INTO public.distribuicao_historico (pipeline_lead_id, corretor_id, segmento_id, acao, motivo_rejeicao, created_at)
      VALUES (p_lead_id, NULL, NULL, 'bloqueado', 'empreendimento_inativo', v_now);

      RETURN jsonb_build_object(
        'success', false,
        'reason', 'empreendimento_inativo',
