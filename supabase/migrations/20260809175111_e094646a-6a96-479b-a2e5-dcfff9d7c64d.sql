CREATE OR REPLACE FUNCTION public.rpc_perf_funil(p_start date, p_end date, p_gerente_id uuid DEFAULT NULL::uuid, p_user_id uuid DEFAULT NULL::uuid)
 RETURNS TABLE(corretor_auth_id uuid, corretor_nome text, avatar_url text, equipe text, equipe_atual text, gerente_auth_id uuid, corretor_ativo boolean, presenca_dias integer, presenca_faltas integer, presenca_saidas integer, dias_uteis integer, dias_uteis_decorridos integer, leads_recebidos bigint, pipeline_ativo bigint, descartes bigint, visitas_total bigint, visitas_agendadas bigint, visitas_realizadas bigint, visitas_no_show bigint, negocios_abertos bigint, vgv_gerado numeric, vendas numeric, vgv_assinado numeric)
 LANGUAGE plpgsql
...
DECLARE
  ...
  v_dias_uteis_dec integer;
  v_end_dec date := LEAST(p_end, (now() AT TIME ZONE 'America/Sao_Paulo')::date);
BEGIN
  ...
  IF v_end_dec < p_start THEN
    v_dias_uteis_dec := 0;
  ELSE
    SELECT COUNT(*)::int INTO v_dias_uteis_dec
    FROM generate_series(p_start, v_end_dec, interval '1 day') d
    WHERE EXTRACT(DOW FROM d) BETWEEN 1 AND 5
      AND NOT EXISTS (SELECT 1 FROM public.feriados f WHERE f.data = d::date);
  END IF;
  ...
  pres AS (
    SELECT p.user_id AS auth_id,
           ...
           COUNT(DISTINCT rp.data) FILTER (WHERE rp.status = 'falta')::int AS faltas,
           COUNT(DISTINCT rp.data) FILTER (WHERE rp.status = 'saiu')::int AS saidas
    FROM public.roleta_presencas rp
    JOIN public.profiles p ON p.id = rp.corretor_id
    WHERE rp.data BETWEEN p_start AND p_end
      AND EXTRACT(DOW FROM rp.data) BETWEEN 1 AND 5
      AND NOT EXISTS (SELECT 1 FROM public.feriados f WHERE f.data = rp.data)
    GROUP BY p.user_id
  ),
  vis AS (
    SELECT vi.corretor_auth_id AS auth_id, vi.equipe,
           COUNT(DISTINCT vi.visita_id) AS total
  ...
  neg_ativo AS (
    ...
    FROM public.negocios n
    JOIN public.profiles pf ON pf.id = n.corretor_id
    WHERE n.fase IN ('em_negociacao', 'contrato')
      AND n.motivo_queda IS NULL
    GROUP BY pf.user_id
  )
  SELECT
    m.corretor_auth_id,
    ...
    m.gerente_auth_id,
    m.corretor_ativo,
    CASE WHEN m.equipe IS NOT DISTINCT FROM m.equipe_atual THEN LEAST(COALESCE(pres.dias, 0), v_dias_uteis_dec) ELSE 0 END,
    CASE WHEN m.equipe IS NOT DISTINCT FROM m.equipe_atual THEN COALESCE(pres.faltas, 0) ELSE 0 END,
    CASE WHEN m.equipe IS NOT DISTINCT FROM m.equipe_atual THEN COALESCE(pres.saidas, 0) ELSE 0 END,
    v_dias_uteis,
    v_dias_uteis_dec,
    m.leads_recebidos,
    ...
    GREATEST(COALESCE(vis.total, 0), m.visitas_realizadas + m.visitas_no_show),
    m.visitas_agendadas,
    m.visitas_realizadas,
    m.visitas_no_show,
    CASE WHEN m.equipe IS NOT DISTINCT FROM m.equipe_atual THEN COALESCE(neg.total, 0) ELSE 0 END,
    CASE WHEN m.equipe IS NOT DISTINCT FROM m.equipe_atual THEN COALESCE(neg_ativo.vgv, 0) ELSE 0 END,
    m.vendas,
    m.vgv_assinado
  FROM m
  LEFT JOIN public.profiles pr ON pr.user_id = m.corretor_auth_id
  LEFT JOIN pres ON pres.auth_id = m.corretor_auth_id
  LEFT JOIN vis ON vis.auth_id = m.corretor_auth_id AND vis.equipe IS NOT DISTINCT FROM m.equipe
  ...
  LEFT JOIN neg_ativo ON neg_ativo.auth_id = m.corretor_auth_id
