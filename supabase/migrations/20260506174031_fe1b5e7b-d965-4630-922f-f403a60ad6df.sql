...
BEGIN
  ...
  END IF;
  UPDATE public.pipeline_leads SET corretor_id = v_chosen_profile_id, aceite_status = 'aguardando_aceite',
    aceite_expira_em = v_expire_at, distribuido_em = v_now, updated_at = v_now WHERE id = p_lead_id;
  UPDATE public.roleta_fila SET ultima_distribuicao_at = v_now WHERE id = v_chosen_fila_id;
  INSERT INTO public.roleta_distribuicoes (lead_id, corretor_id, segmento_id, janela, enviado_em, expira_em, status)
  VALUES (p_lead_id, v_chosen_auth_id, v_segmento_id, v_target_janela, v_now, v_expire_at, 'aguardando');
