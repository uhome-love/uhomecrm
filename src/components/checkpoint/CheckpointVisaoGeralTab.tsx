...
export default function CheckpointVisaoGeralTab({ teamUserIds, teamNameMap }: Props) {
  ...
  const loadData = useCallback(async () => {
    ...
    if (parados.length > 0) {
      newAlerts.push({ id: "negocios-parados", type: "warning", icon: "⏸️", message: `${parados.length} negócios parados há 5+ dias (VGV: ${fmtCurrency(parados.reduce((s, n) => s + Number(n.vgv_estimado || 0), 0))})`, action: { label: "Ver negócios", route: "/pipeline-negocios" } });
    }
      ...
      {negociosParados.length > 0 && (
        <div className="bg-card border border-border rounded-xl shadow-card overflow-hidden">
          <div className="flex items-center justify-between px-4 py-2.5 border-b border-border">
            ...
            <Button variant="ghost" size="sm" className="text-[10px] h-6 gap-1" onClick={() => navigate("/pipeline-negocios")}>
