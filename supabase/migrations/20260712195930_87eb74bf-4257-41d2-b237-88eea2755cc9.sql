...
BEGIN
  ...
  desc_ev AS (
    SELECT pl.corretor_id AS auth_id, COUNT(DISTINCT h.pipeline_lead_id)::int AS descartes
    FROM pipeline_historico h
    JOIN pipeline_leads pl ON pl.id = h.pipeline_lead_id
    WHERE pl.corretor_id = ANY(v_team_auth)
      AND h.stage_novo_id IN (v_descarte, v_caiu)
      AND (h.created_at AT TIME ZONE 'America/Sao_Paulo')::date BETWEEN p_start AND p_end
    GROUP BY 1
  ),
  vendas AS (
  ...
  corretores AS (
    SELECT
      ...
      COALESCE(de.descartes,0)          AS descartes,
    ...
    LEFT JOIN desc_ev de ON de.auth_id   = b.auth_id
