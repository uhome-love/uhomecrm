...
export default function PlacarDoDia() {
  ...
  const handleMetaChange = (val: number) => {
    ...
    setMeta(v);
    localStorage.setItem("placar_meta", String(v));
  };
    ...
    try {
      ...
      for (const [key, userIds] of Object.entries(equipeIds)) {
        ...
        if (visitasEquipe.length > 0) {
          ...
          const hora = new Date(maisRecente.created_at).toLocaleTimeString("pt-BR", { hour: "2-digit", minute: "2-digit", timeZone: "America/Sao_Paulo" });
          ultimaPorEquipe[key] = {
      ...
      const feedVisitas = [...todasVisitas]
        ...
        .map(v => {
          ...
          const primeiroNome = (nomeMap[v.corretor_id] || "—").split(" ")[0];
          return {
    ...
    <>
      ...
      <div style={{
        ...
        <div style={{ padding: "4px 32px 6px", background: "#ffffff06", borderBottom: "1px solid #ffffff14", flexShrink: 0 }}>
          <div style={{
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
              ...
              <span style={{ fontSize: "clamp(14px, 2vw, 24px)", color: "#ffffff55", fontWeight: 400 }}>/</span>
              {isAdminMode ? (
                <input
                  type="number"
                  value={meta}
                  onChange={e => handleMetaChange(Number(e.target.value))}
                  style={{ width: 60, background: "transparent", border: "1px solid #ffffff33", borderRadius: 6, color: "#fff", padding: "2px 8px", fontSize: 14, fontFamily: "'Bebas Neue', 'Impact', sans-serif" }}
                />
              ) : (
                <span style={{
                  fontSize: "clamp(20px, 3vw, 36px)",
        ...
        <div style={{ display: "flex", gap: 10, padding: "8px 20px", flex: 1, minHeight: 0, overflow: "hidden" }}>
          ...
          {/* Right: Feed lateral (260px fixo, esconde < 1024px) */}
          <div className="placar-feed-lateral" style={{
            width: 260,
            flexShrink: 0,
            background: "#0a0a18",
                  ...
                  return (
                    ...
                    >
                      <div style={{
                        ...
                        boxShadow: `0 0 6px ${corBolinha}88`,
                        flexShrink: 0,
                        marginTop: 3,
                      }} />
