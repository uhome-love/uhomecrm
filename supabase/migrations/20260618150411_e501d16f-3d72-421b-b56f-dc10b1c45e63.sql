CREATE OR REPLACE VIEW public.v_kpi_negocios AS
 SELECT COALESCE(n.auth_user_id, p.user_id) AS auth_user_id,
    ...
    n.corretor_id AS profile_id,
        CASE
            WHEN (n.fase = ANY (ARRAY['proposta'::text, 'negociacao'::text, 'documentacao'::text])) THEN 1
            ELSE 0
        END AS conta_proposta,
        CASE
            WHEN (n.fase = 'vendido'::text) THEN 1
            ELSE 0
        END AS conta_venda,
        CASE
            WHEN (n.fase = ANY (ARRAY['perdido'::text, 'cancelado'::text, 'distrato'::text])) THEN 1
            ELSE 0
