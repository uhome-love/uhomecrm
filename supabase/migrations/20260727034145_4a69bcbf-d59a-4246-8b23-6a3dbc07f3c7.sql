...
CREATE OR REPLACE FUNCTION public.rpc_perf_dashboard(
  p_inicio date DEFAULT (((now() AT TIME ZONE 'America/Sao_Paulo'::text))::date - '30 days'::interval),
  p_fim    date DEFAULT ((now() AT TIME ZONE 'America/Sao_Paulo'::text))::date
)
RETURNS jsonb
LANGUAGE plpgsql
VOLATILE
...
BEGIN
  ...
  -- NOTA MAPPING (auditado 27/07/2026):
  --   negocios.corretor_id            -> profiles.id       (profile_id)
  --   roleta_credenciamentos.corretor_id -> profiles.id    (profile_id)
  --   pipeline_leads.corretor_id      -> profiles.user_id  (auth_user_id)
  --   visitas.corretor_id             -> profiles.user_id  (auth_user_id)
  --   oferta_ativa_tentativas.corretor_id -> profiles.user_id (auth_user_id)
