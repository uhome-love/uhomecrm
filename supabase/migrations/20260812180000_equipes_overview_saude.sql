-- Aba Equipes (nova gestão): adiciona contagens de SAÚDE por toque
-- (em_dia/atencao/desatualizado/estagnado) na visão executiva, por escritório,
-- gestor e corretor. Usa a SSOT public.lead_saude_status (mesma saúde do resto
-- do CRM). Add-only: mantém todos os campos antigos (não quebra o frontend).
...
BEGIN
  ...
  saude_corretor AS (SELECT la.corretor_auth,
      count(*) FILTER (WHERE la.saude = 'verde')::int AS em_dia,
      count(*) FILTER (WHERE la.saude = 'ambar')::int AS atencao,
      count(*) FILTER (WHERE la.saude = 'vermelho')::int AS desatualizado,
      count(*) FILTER (WHERE la.saude = 'estagnado')::int AS estagnado
    FROM leads_ativos la GROUP BY la.corretor_auth),
  ...
  corretor_row AS (SELECT ac.gerente_id, ac.corretor_auth, p.id AS profile_id, p.nome,
      ...
      COALESCE(sc.em_dia, 0) AS em_dia, COALESCE(sc.atencao, 0) AS atencao,
      COALESCE(sc.desatualizado, 0) AS desatualizado, COALESCE(sc.estagnado, 0) AS estagnado,
    ...
    LEFT JOIN saude_corretor sc ON sc.corretor_auth = ac.corretor_auth
  ...
  gestor_oper AS (SELECT cr.gerente_id, count(*)::int AS qtd_corretores,
      ...
      COALESCE(sum(cr.em_dia), 0)::int AS em_dia,
      COALESCE(sum(cr.atencao), 0)::int AS atencao,
      COALESCE(sum(cr.desatualizado), 0)::int AS desatualizado,
      COALESCE(sum(cr.estagnado), 0)::int AS estagnado,
      ...
      jsonb_agg(jsonb_build_object('auth_id', cr.corretor_auth, 'profile_id', cr.profile_id,
        ...
        'em_dia', cr.em_dia, 'atencao', cr.atencao, 'desatualizado', cr.desatualizado, 'estagnado', cr.estagnado,
  ...
  gestor_final AS (SELECT go.gerente_id, gp.id AS profile_id, gp.nome, gp.avatar_url,
      go.qtd_corretores, go.total_leads, go.atrasados, go.em_dia, go.atencao, go.desatualizado, go.estagnado,
      go.negocios, go.vgv_pipeline_ativo,
  ...
  SELECT jsonb_build_object(
    'escritorio', jsonb_build_object(
      ...
      'em_dia', COALESCE((SELECT sum(em_dia) FROM gestor_final), 0),
      'atencao', COALESCE((SELECT sum(atencao) FROM gestor_final), 0),
      'desatualizado', COALESCE((SELECT sum(desatualizado) FROM gestor_final), 0),
      'estagnado', COALESCE((SELECT sum(estagnado) FROM gestor_final), 0),
    ...
    'gestores', COALESCE((SELECT jsonb_agg(jsonb_build_object(
        ...
        'atrasados', gf.atrasados, 'em_dia', gf.em_dia, 'atencao', gf.atencao,
        'desatualizado', gf.desatualizado, 'estagnado', gf.estagnado,
        'negocios', gf.negocios, 'vgv_assinado_mes', gf.vgv_assinado_mes,
