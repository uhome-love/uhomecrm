...
BEGIN
  ...
  v_nova_obs := concat(
    ...
    CASE WHEN COALESCE(trim(v_empreend),'') <> '' AND COALESCE(trim(v_lead.empreendimento),'') <> ''
              AND lower(trim(v_empreend)) IS DISTINCT FROM lower(trim(v_lead.empreendimento))
        ...
        IF v_emp_mudou THEN
          UPDATE public.pipeline_leads
             SET reengajamento_status = 'respondeu_sim',
                 ...
                 empreendimento_canonico_id = COALESCE(v_emp_canon_id, empreendimento_canonico_id),
                 observacoes = concat(
