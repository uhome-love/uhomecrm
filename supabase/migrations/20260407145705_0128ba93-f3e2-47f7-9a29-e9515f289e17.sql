...
-- Step 1: Drop BOTH overloads of distribuir_lead_atomico
DROP FUNCTION IF EXISTS public.distribuir_lead_atomico(uuid, text, uuid, boolean);
DROP FUNCTION IF EXISTS public.distribuir_lead_atomico(uuid, text, boolean, uuid);

-- Step 2: Create the single unified function with force fallback and detailed reasons
...
BEGIN
  ...
  -- Resolve segment via roleta_campanhas (same as trigger)
  ...
  -- ATTEMPT 1: Normal distribution (respects timeout/rejection blocks)
  ...
  -- ATTEMPT 2: Force fallback (CEO dispatch only) — bypass timeout/rejection blocks
  IF v_chosen_auth_id IS NULL AND p_force = true THEN
    -- Count total brokers in window to diagnose reason
    ...
    -- Count how many are blocked by history
    ...
    -- If all brokers are blocked by history, force redistribute (skip only the most recent one)
...
-- Step 3: Update the trigger function to also use roleta_campanhas consistently
-- (it already does, but let's ensure the latest version is active)
