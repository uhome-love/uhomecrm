...
BEGIN
  ...
  vis AS (
    SELECT
      (a_realizar + confirmadas + realizadas + no_show) AS criadas,
      a_realizar, confirmadas, realizadas, no_show
    FROM (
      SELECT
  ...
  SELECT jsonb_build_object(
    'kpis_top', jsonb_build_object(
      ...
      'visitas_agendadas', (SELECT criadas FROM vis),
      'visitas_total', (SELECT criadas FROM vis),
