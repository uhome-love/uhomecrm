...
BEGIN
  ...
  -- ============================================================
  -- ROUND-ROBIN ESTRITO 1 A 1 POR TURNO
  -- Conta quantos leads cada corretor já recebeu no MESMO turno
  -- (today + janela + segmento_id) via roleta_distribuicoes.
  -- Em modo automático (p_force=false), só considera os corretores
  -- com a MENOR contagem do pool — bloqueando repetições até que
  -- todos os credenciados elegíveis tenham recebido pelo menos um.
  -- Em disparo manual do CEO (p_force=true), mantém o critério
  -- antigo (último a receber) sem trava de min-count.
  -- ============================================================
  WITH eligiveis AS (
  ...
  filtrados AS (
    ...
    WHERE p_force = true
       OR recebidos_no_turno = (SELECT MIN(recebidos_no_turno) FROM eligiveis)
  )
  SELECT sub.fila_id, sub.profile_id, sub.auth_id INTO v_chosen_fila_id, v_chosen_profile_id, v_chosen_auth_id
  FROM (
