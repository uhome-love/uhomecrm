-- Fix: RPC was setting 'pendente_aceite' but system filters expect 'aguardando_aceite'
UPDATE public.pipeline_leads
   SET aceite_status = 'aguardando_aceite'
 WHERE aceite_status = 'pendente_aceite';
...
BEGIN
  ...
  -- Determine segment
  -- Casa Tua / Geral: ignore segment (all active brokers)
  IF v_emp_lower LIKE '%casa tua%' OR v_emp_lower = '' OR v_emp_lower = 'geral' THEN
    v_ignorar_segmento := true;
  ELSE
    -- Try to match campaign by empreendimento
    SELECT rc.segmento_id, rs.nome INTO v_segmento_id, v_segmento_nome
  ...
  -- corretor_id in roleta_fila is profile id; resolve to auth.users.id for pipeline_leads.corretor_id
