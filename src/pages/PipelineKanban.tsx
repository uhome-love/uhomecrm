...
export default function PipelineKanban() {
  ...
  const { data: kanbanTarefasMap = {}, isLoading: tarefasLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      for (const row of rows) {
        const nextTask: ProximaTarefa = { id: row.id, titulo: row.titulo || "Tarefa", tipo: row.tipo, subtipo: row.subtipo ?? null, vence_em: row.vence_em, hora_vencimento: row.hora_vencimento, origem: row.origem ?? null };
  ...
  return (
    ...
    <div
      className="flex flex-col w-full max-w-full min-w-0 overflow-hidden bg-[#f0f0f5] dark:bg-background px-2 pt-2"
