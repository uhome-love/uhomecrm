...
BEGIN
  ...
  IF v_is_casatua THEN
    v_seg_id := v_seg_casa_tua;
    v_empreend := 'Casa Tua';
    v_foco_label := 'Casa Tua (FOCO)';
  ELSIF v_is_vivid THEN
    -- Vivid Terrace é Produto Foco (S5); empreendimento="Vivid Terrace" resolve para FOCO na roleta via roleta_campanhas
    v_seg_id := v_lead.segmento_id; -- pipeline_segmentos não tem "Produto Foco"; segmento operacional vem da roleta pelo empreendimento
