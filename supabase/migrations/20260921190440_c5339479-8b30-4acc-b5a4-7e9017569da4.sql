...
BEGIN
  ...
  -- Guarda: lead em etapa final (venda/contrato) ou com negocio ganho ATIVO nao volta a circular.
  -- Negocios ganhos arquivados/perdidos (sem assinatura) NAO bloqueiam mais a distribuicao.
  IF EXISTS (
    SELECT 1 FROM public.pipeline_leads pl
    JOIN public.pipeline_stages ps ON ps.id = pl.stage_id
    WHERE pl.id = p_lead_id AND ps.tipo IN ('venda','contrato_gerado')
  ) OR EXISTS (
    SELECT 1 FROM public.negocios n
    WHERE n.pipeline_lead_id = p_lead_id AND n.fase = 'ganho' AND n.status = 'ativo'
  ) THEN
    RETURN jsonb_build_object('success', false, 'reason', 'lead_ganho');
  END IF;
  ...
  -- Regra fixa: produto identificado exige corretor ALOCADO e ativo na roleta.
  -- Sem alocado ativo => Fila do CEO (nunca cai no rodizio por segmento).
  IF v_emp_canonico_id IS NOT NULL THEN
    ...
    SELECT fila_id, profile_id, auth_id, recebidos_no_produto, (SELECT count(*) FROM elegiveis)
    ...
    IF v_chosen_fila_id IS NOT NULL THEN
      v_pool := 'alocado';
    ELSIF p_force THEN
      v_use_segmento := TRUE;
    ELSE
  ...
  IF v_chosen_fila_id IS NULL THEN
    ...
    UPDATE public.pipeline_leads
    SET aceite_status = 'pendente_distribuicao',
        motivo_pendencia = v_failure_reason,
        updated_at = v_now
    WHERE id = p_lead_id;
    ...
    INSERT INTO public.distribuicao_historico
      (pipeline_lead_id, corretor_id, segmento_id, acao, motivo_rejeicao, created_at, empreendimento_canonico_id, pool, pool_size)
    VALUES
      (p_lead_id, NULL, v_segmento_id, 'fila_ceo', v_failure_reason, v_now, v_emp_canonico_id, 'fila_ceo', 0);
