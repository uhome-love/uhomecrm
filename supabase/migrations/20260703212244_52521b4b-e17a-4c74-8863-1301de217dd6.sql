...
BEGIN
  ...
  IF v_phone8 IS NOT NULL AND length(v_phone8) = 8 THEN
    ...
    IF FOUND THEN
      ...
      -- EXCEÇÃO: disparo do Lake Baikal SEMPRE vai para a Fila do CEO (padrão definido),
      -- mesmo que o lead já esteja ativo com um corretor.
      ...
      -- Lead existe (INATIVO: arquivado/descartado) OU disparo Lake Baikal (override):
      -- reaproveita via fluxo padrão → Fila do CEO (arranca do corretor atual, se houver)
      UPDATE public.oferta_ativa_leads
         SET status = 'reativado', updated_at = now()
       WHERE id = p_oa_lead_id;
