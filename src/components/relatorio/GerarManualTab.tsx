...
export default function GerarManualTab({ team, gerenteNome }: Props) {
  ...
  const { data: periodData, isLoading: loadingPeriod } = useQuery({
    ...
    queryFn: async (): Promise<PeriodData> => {
      ...
      const negociosAtivos = negocios.filter(n => n.fase && n.status === "ativo" && n.fase !== "ganho").length;
      ...
      const perdidos = negocios.filter(n => n.fase && n.status === "perdido").length;
      const vgvAndamento = negocios
        .filter(n => n.fase && n.status === "ativo" && n.fase !== "ganho")
        .reduce((s, n) => s + (Number(n.vgv_estimado) || 0), 0);
      const vgvGerado = negocios
        .filter(n => n.fase && n.status === "ativo")
        .reduce((s, n) => s + (Number(n.vgv_estimado) || 0), 0);
