...
CREATE POLICY "Anyone authenticated can view feriados" ON public.feriados
  FOR SELECT TO authenticated USING (true);

CREATE POLICY "Service role can manage feriados" ON public.feriados
  FOR ALL TO service_role USING (true) WITH CHECK (true);

-- 2. Populate 2026 holidays
INSERT INTO public.feriados (data, descricao) VALUES
  ...
  ('2027-09-20', 'Revolução Farroupilha'),
...
-- 3. Replace distribuir_lead_atomico with improved version
...
BEGIN
  ...
  INSERT INTO public.roleta_distribuicoes (
    lead_id, corretor_id, segmento_id, janela, status, enviado_em, expira_em
  ) VALUES (
    p_lead_id, v_chosen_profile_id, v_segmento_id, v_target_janela, 'aguardando', v_now, v_expire_at
  );
