...
BEGIN
  ...
  -- Resolve o produto canônico a partir do nome do empreendimento (nome exato + alias).
  v_emp_canon_id := NULL;
  IF COALESCE(trim(v_empreend), '') <> '' THEN
    ...
    IF v_emp_canon_id IS NULL THEN
      ...
      WHERE ec2.ativo = true
        AND lower(trim(ea.alias)) = lower(trim(v_empreend))
      LIMIT 1;
    END IF;
  END IF;

  -- Se o canônico atual do lead continua batendo com o nome, preserva.
