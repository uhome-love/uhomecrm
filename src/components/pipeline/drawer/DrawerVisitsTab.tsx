...
function VisitaCard({ visita, isFuture }: { visita: Visita; isFuture: boolean }) {
  ...
  const statusCls = STATUS_STYLE[status] ?? "bg-zinc-100 text-muted-foreground";
  const statusLabel = STATUS_LABEL[status] ?? status;

  return (
    <div className="rounded-xl border border-border p-4 mb-3 relative bg-card">
      ...
      <div className="flex items-start gap-3 mb-3">
        <div className={`w-14 rounded-xl p-2 text-center flex-shrink-0 ${
          isFuture
            ...
            : "bg-zinc-100 border border-border"
        }`}>
          <div className={`text-[11px] font-semibold uppercase ${isFuture ? "text-emerald-600" : "text-muted-foreground"}`}>
            {diaSemana}
          </div>
          <div className={`text-[22px] font-bold leading-none my-0.5 ${isFuture ? "text-emerald-700" : "text-zinc-700"}`}>
            {dia}
          </div>
          <div className="text-[10px] text-muted-foreground uppercase">{mes}</div>
        </div>

        <div className="flex-1 min-w-0">
          <div className="text-xs text-muted-foreground font-medium mb-1 flex items-center gap-1.5 flex-wrap">
            {hora && <span>{hora}</span>}
      ...
      {visita.observacoes && (
        <div className="text-xs text-muted-foreground bg-zinc-50 rounded-md p-2 mb-2 leading-relaxed">
          {visita.observacoes}
        </div>
      )}
      ...
      <div className="flex gap-1.5 pt-2.5 border-t border-zinc-100 flex-wrap">
        {isFuture ? (
          ...
          visita.resultado_visita && (
            <div className="text-[11px] text-muted-foreground px-2.5 py-1 rounded-md bg-zinc-50 border border-border">
