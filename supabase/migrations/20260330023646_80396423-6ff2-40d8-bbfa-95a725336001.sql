...
BEGIN
  ...
  -- Tarefas atrasadas: compara vence_em com a data/hora atual em BRT
  SELECT
    ...
    CONCAT('Tarefa atrasada há ',
      GREATEST(1, ROUND(EXTRACT(EPOCH FROM (v_now_brt - (pt.vence_em::timestamp + COALESCE(pt.hora_vencimento, '00:00:00')::time))) / 3600, 0))::TEXT,
      'h: ', pt.titulo
    ) AS descricao,
  ...
  WHERE pt.responsavel_id = p_corretor_id
    ...
    -- Compara a data de vencimento + hora com o horário atual em BRT
    AND (pt.vence_em::timestamp + COALESCE(pt.hora_vencimento, '00:00:00')::time) < v_now_brt
    AND (v_descarte_stage_id IS NULL OR pl.stage_id != v_descarte_stage_id)
    AND (v_convertido_stage_id IS NULL OR pl.stage_id != v_convertido_stage_id)
