-- BUG 1: reciclar_leads_sem_contato insere em colunas inexistentes em pipeline_historico (causa erro a cada 1 min nas logs do lead-escalation)
-- BUG 2: distribuir_lead_roleta envia push para URL antiga '/aceite-leads' (correto: '/aceite')
...
BEGIN
  ...
  LOOP
    ...
    -- FIX: usar colunas reais de pipeline_historico
    INSERT INTO pipeline_historico (pipeline_lead_id, movido_por, observacao)
    VALUES (
      r.id,
      r.corretor_id,
...
-- Fix push URL para a tela correta
  ...
  BEGIN
    PERFORM net.http_post(
      ...
      body := jsonb_build_object(
        ...
        'url', '/aceite?lead=' || p_pipeline_lead_id::text
      )
    );
