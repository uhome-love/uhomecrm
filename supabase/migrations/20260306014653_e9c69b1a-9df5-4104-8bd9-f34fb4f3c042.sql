...
BEGIN
  ...
  -- ═══ AUTO-SYNC: Always update checkpoint_lines with today's totals ═══
  SELECT tm.id, tm.gerente_id INTO v_team_member
  FROM team_members tm
  WHERE tm.user_id = p_corretor_id AND tm.status = 'ativo'
  LIMIT 1;
  ...
  RETURN jsonb_build_object('success', true, 'resultado', COALESCE(p_resultado, 'com_interesse'), 'pontos', COALESCE(v_pontos, 3), 'visita_marcada', p_visita_marcada, 'attempt_id', v_attempt_id);
END;
$function$
