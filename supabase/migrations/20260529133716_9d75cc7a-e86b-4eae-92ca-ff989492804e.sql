...
BEGIN
  ...
  SELECT count(*)::int INTO v_pipeline_ativo
  FROM pipeline_leads
  WHERE corretor_id = ANY(v_team_auth)
    AND arquivado = false
    AND COALESCE(status_lead,'') NOT IN ('inativado','descartado');

  SELECT count(*)::int INTO v_total_ativos
  FROM pipeline_leads
  WHERE corretor_id = ANY(v_team_auth) AND arquivado = false;

  SELECT count(*)::int INTO v_atualizados_48h
  FROM pipeline_leads
  WHERE corretor_id = ANY(v_team_auth) AND arquivado = false
    AND updated_at >= now() - interval '48 hours';

  SELECT count(DISTINCT pl.id)::int INTO v_leads_com_visita
  ...
  WHERE pl.corretor_id = ANY(v_team_auth)
    ...
    AND EXISTS (SELECT 1 FROM visitas v WHERE v.pipeline_lead_id = pl.id);
  ...
  v_extras := jsonb_build_object(
    ...
    'conversao_lead_visita_pct', CASE WHEN (v_core->'leads'->>'recebidos')::int > 0
        THEN round(v_leads_com_visita * 100.0 / (v_core->'leads'->>'recebidos')::int, 1) END,
    'taxa_atualizacao_48h', CASE WHEN v_total_ativos > 0
        THEN round(v_atualizados_48h * 100.0 / v_total_ativos, 1) END
  );
...
CREATE OR REPLACE FUNCTION public.get_relatorio_vendas(p_gestor_id uuid, p_start date, p_end date, p_prev_start date DEFAULT NULL::date, p_prev_end date DEFAULT NULL::date)
 RETURNS jsonb
 LANGUAGE plpgsql
 STABLE SECURITY DEFINER
 SET search_path TO 'public'
