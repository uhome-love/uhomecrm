...
DECLARE
  ...
  v_seg_casa_tua uuid := 'c8b24415-3dc1-4f65-aae1-f308ef02cb7a'; -- pipeline_segmentos "Médio-Alto Padrão" (contém Casa Tua); empreendimento="Casa Tua" resolve para FOCO na roleta
...
BEGIN
  ...
  v_seg_id := CASE WHEN v_is_casatua THEN v_seg_casa_tua ELSE v_lead.segmento_id END;
  v_empreend := CASE WHEN v_is_casatua THEN 'Casa Tua' ELSE v_lead.empreendimento END;
  ...
  -- Cancela tarefas pendentes antigas
  ...
  v_nova_obs := concat(
    ...
    CASE WHEN v_is_casatua THEN ' Interesse atual: Casa Tua (segmento FOCO).' ELSE '' END,
  ...
  RETURN jsonb_build_object(
    ...
    'template', v_tpl,
    'segmento_foco', CASE WHEN v_is_casatua THEN 'Casa Tua (FOCO)' ELSE NULL END,
