DROP FUNCTION IF EXISTS public.get_corretor_pre_estagnacao();
...
AS $function$
  ...
  SELECT
    ...
    CASE
      WHEN c.estagnado_aviso_em IS NOT NULL AND c.estagnado_prazo_em > now() THEN 'em_aviso'
      ELSE 'proximo'
    END AS categoria
  FROM calc c
  WHERE c.prazo_real <= now() + interval '5 days'
