...
BEGIN
  ...
  -- Roteamento explícito por ORIGEM: Imovelweb / Site → S3 Avulso
  IF v_lead_origem_lower LIKE '%imovelweb%'
    ...
    v_matched_campaign := TRUE; -- impede que o fallback de origens_gerais/avulso sobrescreva
  END IF;
  ...
  IF NOT v_matched_campaign AND v_emp_lower <> '' THEN
    ...
    ORDER BY
      (lower(trim(empreendimento)) = v_emp_lower) DESC, -- match exato primeiro
      length(empreendimento) DESC                        -- depois o nome mais específico
    LIMIT 1;
