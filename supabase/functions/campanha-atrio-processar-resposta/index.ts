...
// Bloqueia criação/reativação de lead via campanha Átrio quando já existe lead
// com mesmo telefone em estado "intocável":
 ...
 * Ordem de avaliação: arquivado → stage → negocio_id (primeiro motivo encontrado vence).
 *
 * @param supabase  cliente Supabase (qualquer cliente com `.from(...).select(...)`)
 * @param telefone  telefone bruto recebido do WhatsApp (string vinda do payload `from`)
 */
