...
AS $function$
  WITH params AS (
    ...
    JOIN oferta_ativa_listas ol ON ol.id = oal.lista_id
    WHERE oal.telefone_normalizado IS NOT NULL
      AND oal.status IN ('na_fila','em_cooldown','aproveitado')
      AND ol.status = 'liberada'
      AND (ol.expira_em IS NULL OR ol.expira_em > now())
  ), bruto AS (
...
BEGIN
  ...
  WITH pl AS (
    ...
    WHERE b.opt_out = false
      ...
      -- Higiene: nunca liberar quem tem lead ativo no pipeline nem inativado (definitivo/arquivado)
      ...
      -- Descarte reengajável: só entra se permitido e fora da janela mínima
      AND NOT (
