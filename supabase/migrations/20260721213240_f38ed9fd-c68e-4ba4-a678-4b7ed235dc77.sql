...
-- Fix 1: credenciar_por_alocacao — status válido no CHECK de corretor_disponibilidade
...
BEGIN
  ...
  -- FIX: status 'na_empresa' respeita o CHECK constraint da tabela
  INSERT INTO corretor_disponibilidade (user_id, na_roleta, status, updated_at)
  VALUES (v_auth_user_id, true, 'na_empresa', now())
  ...
  RETURN json_build_object('success', true, 'credenciamento_id', v_cred_id,
    'message', 'Credenciamento aprovado e adicionado à fila!');
END;
$function$;

-- Fix 2: fechar a porta legada — corretor não pode mais inserir direto em roleta_credenciamentos
DROP POLICY IF EXISTS "Corretores insert own credenciamentos" ON public.roleta_credenciamentos;
