...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // ── Auth (required, no source-based bypass) ──
    ...
    if (!webhookSecret) {
      L.warn("LANDING_WEBHOOK_SECRET not configured — rejecting request");
      return new Response(JSON.stringify({ error: "Webhook not configured" }), {
        status: 503, headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
    if (!isValidSecret && !isValidServiceRole) {
    ...
    if (telefone) {
      ...
      if (existing) {
        ...
        if (isDiscarded) {
          ...
          L.info("Lead descartado reenviado para a roleta", { leadId: existing.id, distributed: reac.distributed });
          return new Response(
    ...
    return new Response(
      JSON.stringify({ success: true, lead_id: insertedLead.id, empreendimento, distributed: distribution.success, trace_id: traceId }),
      { headers: { ...corsHeaders, "Content-Type": "application/json" } }
    );
  } catch (err) {
