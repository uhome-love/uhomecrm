DROP VIEW IF EXISTS public.v_fato_visita_principal;
DROP VIEW IF EXISTS public.v_kpi_visitas_principal;
DROP FUNCTION IF EXISTS public.__noop_visitas_ssot();
...
SELECT corretor_id AS auth_user_id,
    ...
    empreendimento,
    origem,
        CASE WHEN (status = ANY (ARRAY['marcada'::text, 'confirmada'::text, 'realizada'::text, 'reagendada'::text])) THEN 1 ELSE 0 END AS conta_marcada,
        CASE WHEN (status = 'realizada'::text) THEN 1 ELSE 0 END AS conta_realizada,
        CASE WHEN (status = 'no_show'::text) THEN 1 ELSE 0 END AS conta_no_show,
    cliente_key,
    seq_dia,
    true AS visita_principal_dia
   FROM ranked v
