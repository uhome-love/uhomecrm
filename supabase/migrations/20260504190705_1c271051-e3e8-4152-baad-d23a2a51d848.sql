...
DECLARE
  ...
  v_origens_avulsas TEXT[] := ARRAY['site','site_uhome','imovelweb','imovel_web','imóvelweb'];
...
BEGIN
  ...
  -- 1) Match por empreendimento (campanha)
  IF v_emp_lower <> '' THEN
  ...
  -- 2) Sem match por campanha: roteia leads de Site/ImovelWeb para S3 - Avulso
  IF NOT v_matched_campaign AND v_lead_origem_lower = ANY(v_origens_avulsas) THEN
    v_segmento_id := v_avulso_segmento_id;
    v_ignora_segmento := FALSE;
  -- 3) Origens marcadas como gerais (ex.: jetimob direto): todos recebem
  ELSIF NOT v_matched_campaign AND v_lead_origem_lower = ANY(v_origens_gerais) THEN
    v_segmento_id := NULL;
    v_ignora_segmento := TRUE;
  -- 4) Sem empreendimento e sem origem reconhecida: também geral
  ELSIF NOT v_matched_campaign AND v_emp_lower = '' THEN
    v_ignora_segmento := TRUE;
    v_segmento_id := NULL;
  END IF;
  ...
  -- ATTEMPT 1: Round-robin estrito 1 a 1
  ...
  IF v_chosen_fila_id IS NULL THEN
    ...
    v_failure_reason := 'no_eligible_brokers';
  ...
  INSERT INTO public.roleta_distribuicoes (lead_id, corretor_id, segmento_id, janela, distribuido_em, expira_em, status)
  VALUES (p_lead_id, v_chosen_profile_id, v_segmento_id, v_target_janela, v_now, v_expire_at, 'pendente');
